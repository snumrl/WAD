...
//     int N = MASS::muscle->GetAnchors().size();
...
// void ShapeRenderer::clearMesh(const MeshShape* meshShape) {
//     std::vector<uint32_t> vbos = meshShapeVbo.at(meshShape);
//     glDeleteBuffers(vbos.size(), vbos.data());
//     meshShapeVbo.erase(meshShape);
// }
...
//         glEnableClientState(GL_VERTEX_ARRAY);
//         if (enableNormals) glEnableClientState(GL_NORMAL_ARRAY);
//         if (enableColors) glEnableClientState(GL_COLOR_ARRAY);

//         int stride = 3*sizeof(float);
//         if (enableNormals) stride += 3*sizeof(float);
//         if (enableColors) stride += 4*sizeof(float);
//         glVertexPointer(3, GL_FLOAT, stride, (void*)0);
//         if (enableNormals) glNormalPointer(GL_FLOAT, stride, (void*)(3*sizeof(float)));
//         if (enableColors) glColorPointer(4, GL_FLOAT, stride, (void*)(6*sizeof(float)));
...
//         glDisableClientState(GL_VERTEX_ARRAY);
//         if (enableNormals) glDisableClientState(GL_NORMAL_ARRAY);
//         if (enableColors) glDisableClientState(GL_COLOR_ARRAY);
//         glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void ShapeRenderer::renderMuscle(Muscle* muscle) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, std::get<0>(bo));
    glBufferSubData(GL_ARRAY_BUFFER, 0, 3*sizeof(float)*numVertices, vertices.data());
    glBufferSubData(GL_ARRAY_BUFFER, 3*sizeof(float)*numVertices, 2*3*sizeof(float)*numVertices, contourNormals.data());

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, std::get<1>(bo));
    glEnableClientState(GL_VERTEX_ARRAY);
    glEnableClientState(GL_NORMAL_ARRAY);
...
void ShapeRenderer::renderMesh(const MeshShape *meshShape, bool drawShadows, float shadowY, const Eigen::Vector4d& color) {
