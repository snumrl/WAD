...
LoadMuscleNN(const std::string& muscle_nn_path)
{
	...
	p::exec(str,mns);
	str = ("num_muscles = "+std::to_string(mCharacter->GetMuscles().size())).c_str();
	p::exec(str,mns);
...
{
	mDevice = mEnv->GetCharacter()->GetDevice();
...
Step()
{
	...
	if(mEnv->GetUseMuscle())
	{
		...
		for(int i=0; i<num; i+=inference_per_sim){
			Eigen::VectorXd mt = mCharacter->GetMuscleTorques();
...
SetTrajectory()
{
	const SkeletonPtr& skel = mCharacter->GetSkeleton();
...
SetFocusing()
{
	if(mFocus)
	{
		mTrans = -mCharacter->GetSkeleton()->getRootBodyNode()->getCOM();
		mTrans[1] -= 0.3;
		mTrans *=1000.0;
	}
}
...
DrawCharacter()
{
	...
	if(mEnv->GetUseMuscle())
		DrawMuscles(mCharacter->GetMuscles());
...
DrawDevice()
{
	if(mCharacter->GetOnDevice())
	{
		DrawSkeleton(mDevice->GetSkeleton());
		DrawDeviceSignals();
		if(mDrawDeviceForce)
			DrawDeviceForce();
	}
...
DrawDeviceForce()
{
	const SkeletonPtr& skel_device = mDevice->GetSkeleton();
...
GetActionFromNN_Device()
{
	...
	np::ndarray state_np = toNumPyArray(state);

	p::object get_action_device = device_nn_module.attr("get_action");
...
{
	if(!mMuscleNNLoaded)
		std::cout << "Muscle Not Loaded" << std::endl;

	Eigen::VectorXd dt = mCharacter->GetDesiredTorques();
